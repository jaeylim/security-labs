...
## ai-wave — AI Security Wave 1기 (CJ올리브네트웍스)
`ai-wave/`
...
└── screenshots/       # 콘솔 실습 스크린샷
```

새 과정이 추가되면 `ai-wave/`와 같은 레벨에 새 과정 폴더(예: `next-course/`)를 만들고, 공통으로 재사용 가능한 자료는 `shared/`에 둡니다.

> 스크린샷에는 계정 ID, ARN, 실제 사용자 정보가 포함되지 않도록 마스킹 후 업로드합니다.
